...
class EventsController extends AgendaAppController {
	...
	function view($id = null) {
		if (!$id) {
			$this->Session->setFlash(__d('agenda','Invalid Event', true), 'default', array('class' => 'error'));
	...
	function add() {
		if (!empty($this->data)) {
			...
			if ($this->Event->save($this->data)) {
				$this->Session->setFlash(__d('agenda','The Event has been saved', true), 'default', array('class' => 'success'));
				$this->redirect(array('action'=>'index','course_id' => $this->data['Event']['course_id']));
			} else {
				$this->Session->setFlash(__d('agenda','The Event could not be saved. Please, try again.', true), 'default', array('class' => 'error'));
			}
		}
	...
	function delete($id = null) {
		if (!$id) {
			$this->Session->setFlash(__d('agenda','Invalid id for Event', true), 'default', array('class' => 'error'));
			$this->redirect(array('action'=>'index'));
		}
		if ($this->Event->del($id)) {
			$this->Session->setFlash(__d('agenda','Event deleted', true), 'default', array('class' => 'success'));
			$this->redirect(array('action'=>'index'));
		}
	}
