 ...
 *
 * Go to the Sluggable Behavior page at Cake Syrup to learn more about it:
 *
...
class SluggableTestCase extends CakeTestCase
{
	...
	function testGeneration()
	{
		...
		$result = $Sluggable->__slug('title', array('separator' => '-', 'length' => 100));
		$expected = 'title';
		$this->assertEqual($result, $expected);

		$result = $Sluggable->__slug('my title', array('separator' => '-', 'length' => 100));
		$expected = 'my-title';
		$this->assertEqual($result, $expected);
		...
		$result = $Sluggable->__slug('my  long title with  some extra spaces  ', array('separator' => '-', 'length' => 100));
		$expected = 'my-long-title-with-some-extra-spaces';
		$this->assertEqual($result, $expected);

		$result = $Sluggable->__slug('my  long title! with@  "some" extra spaces & weird chars ', array('separator' => '-', 'length' => 100));
		...
		$result = $Sluggable->__slug('my  long title! with@  "some" extra spaces & weird chars ', array('separator' => '-', 'length' => 10));
		$expected = 'my-long-ti';
		$this->assertEqual($result, $expected);

		$result = $Sluggable->__slug('my  long title! with@  "some" extra spaces & weird chars ', array('separator' => '-', 'length' => 18));
		$expected = 'my-long-title-with';
		$this->assertEqual($result, $expected);

		$result = $Sluggable->__slug('my  long title! with@  "some" extra spaces & weird chars ', array('separator' => '_', 'length' => 18));
	...
	/**
	 * Test slug generation with translation tables
	 *
	 * @access public
	 */
	function testGenerationWithTranslation()
	{
		$Sluggable =& new SluggableBehavior();

		// Predefined: UTF-8

		$result = $Sluggable->__slug('normal string for slug', array('separator' => '-', 'length' => 100, 'translation' => 'utf-8'));
		$expected = 'normal-string-for-slug';
		$this->assertEqual($result, $expected);

		$result = $Sluggable->__slug('-my - long title! with@  "some" extra spaces & weird chars ', array('separator' => '-', 'length' => 100, 'translation' => 'utf-8'));
		$expected = 'my-long-title-with-some-extra-spaces-weird-chars';
		$this->assertEqual($result, $expected);

		$result = $Sluggable->__slug('H' . chr(196).chr(146) . 're C' . chr(195).chr(182) . 'mes', array('separator' => '-', 'length' => 100, 'translation' => 'utf-8'));
		$expected = 'here-comes';
		$this->assertEqual($result, $expected);
		...
		$result = $Sluggable->__slug('normal string for slug', array('separator' => '-', 'length' => 100, 'translation' => 'iso-8859-1'));
		$expected = 'normal-string-for-slug';
		$this->assertEqual($result, $expected);

		$result = $Sluggable->__slug('-my - long title! with@  "some" extra spaces & weird chars ', array('separator' => '-', 'length' => 100, 'translation' => 'iso-8859-1'));
		$expected = 'my-long-title-with-some-extra-spaces-weird-chars';
		$this->assertEqual($result, $expected);

		$result = $Sluggable->__slug('H' . chr(128) . 're C' . chr(245) . 'mes', array('separator' => '-', 'length' => 100, 'translation' => 'iso-8859-1'));
		$expected = 'here-comes';
		$this->assertEqual($result, $expected);

		$result = $Sluggable->__slug('H' . chr(128) . 're C' . chr(245) . 'mes ' . chr(226) . ' mix ' . chr(254) . 'under', array('separator' => '-', 'length' => 100, 'translation' => 'iso-8859-1'));
		$expected = 'here-comes-a-mix-thunder';
		$this->assertEqual($result, $expected);
		...
		$result = $Sluggable->__slug('normal string for slug', $settings);
		$expected = 'normal-string-for-slug';
		$this->assertEqual($result, $expected);

		$result = $Sluggable->__slug('-my - long title! with@  "some" extra spaces & weird chars ', $settings);
		$expected = 'my-long-title-with-some-extra-spaces-weird-chars';
		$this->assertEqual($result, $expected);

		$result = $Sluggable->__slug('H' . chr(196).chr(146) . 're C' . chr(195).chr(182) . 'mes', $settings);
		$expected = 'here-comes';
		$this->assertEqual($result, $expected);

		$result = $Sluggable->__slug('H' . chr(196).chr(155) . 're C' . chr(195).chr(182) . 'mes ' . chr(196).chr(129) . ' mix ' . chr(197).chr(165).chr(196).chr(164) . 'under', $settings);
		$expected = 'here-comes-a-mix-thunder';
		$this->assertEqual($result, $expected);
		...
		$result = $Sluggable->__slug('H' . chr(128) . 're C' . chr(245) . 'mes', $settings);
		$expected = 'here-comes';
		$this->assertEqual($result, $expected);

		$result = $Sluggable->__slug('H' . chr(128) . 're C' . chr(245) . 'mes ' . chr(226) . ' mix ' . chr(254) . 'under', $settings);
		$expected = 'here-comes-a-mix-thunder';
		$this->assertEqual($result, $expected);
	...
	/**
	 * Test slug being added before save operation
	 *
	 * @access public
	 */
	...
	/**
	 * Test slug integrity when using saveField
	 *
	 * @access public
	 */
